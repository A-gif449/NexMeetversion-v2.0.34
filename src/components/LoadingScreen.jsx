...
//   1. three.js lazy-imported inside useEffect → removed from main bundle (~160 KB win)
//   2. Data ticker paused when tab is hidden → saves CPU on mobile
//   3. All visuals, animations, and logic identical to original
...
export default function LoadingScreen({ onComplete }) {
  ...
  /* ── FIX 1: Lazy-import three.js so it's excluded from the main bundle ── */
  useEffect(() => {
    ...
    import("three").then((THREE) => {
      ...
      renderer.setPixelRatio(1); // Always 1 — biggest GPU win
      ...
      const helix = new THREE.Points(helixGeo, new THREE.PointsMaterial({ size: .22, vertexColors: true, transparent: true, opacity: .85 }));
      scene.add(helix);
      ...
      const torus = new THREE.Mesh(
        new THREE.TorusGeometry(8, .04, 6, 80),
        new THREE.MeshBasicMaterial({ color: ACCENT3, transparent: true, opacity: .35 })
      );
      ...
      for (let i = 0; i < 800 * 3; i++) starPos[i] = (Math.random() - .5) * 200;
      ...
      // Store cleanup for when the effect tears down
      cleanupFns.push(() => {
  ...
  /* ── FIX 2: Data ticker — pause when tab is hidden to save CPU ── */
  const [dataVals, setDataVals] = useState({
    lat: "28.6139", lng: "77.2090", enc: "AES-256",
    fps: "60", ping: "12ms", nodes: "7",
  });
  useEffect(() => {
    const tick = () => {
      setDataVals({
